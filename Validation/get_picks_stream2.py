...
def main():
   ...
   iristest2Path = "..\\SeedData\\IRIS_2018_2020\\test\\*.mseed"
   ...
   dataPath =iristestPath
   dataname = "IRIS"
   plotdir = f'{dataname}_testing' # result folder
   ptime=60
   ...
   overlap =1 # interval between algorithm application
   ...
   pick_modes = ['truth', 'stftcnn2'] # modes selected 
   probe_list = ['stftcnn2'] # modes whose output is plotted directly
   snr_list=[]
   plotpath = os.path.join('results', f'{plotdir}_overlap{overlap}') #
   com.safe_mkdir(plotpath)     
   
   test = Testbench(pick_modes=pick_modes, overlap=overlap, tolerance=0.5) # Testbench object
   ...
   for mode in pick_modes:
     for phase in ['p', 's']:
       if (mode in test.eval_dict) and ('cnn' in mode):
