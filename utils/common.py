...
def save_seed_events(old_dir, new_dir, channels=[], num=0):
    st = obspy.read(old_dir)
    safe_mkdir(new_dir)
    stream_list = stream_to_events(st, channels=channels, remove_duplicates=1, apply_fill=1)
    
    if num: stream_list=stream_list[:num]
    print(len(stream_list), len(stream_list[0]))
    
    for st in stream_list:
       info = get_info(st, mode='multi')
       name = make_trace_info(info)+ '.mseed'
       
       st.write(os.path.join(new_dir, name), format='MSEED')

def save_train_test_events(old_dir, new_dir, f=[], channels=[], num=0, test=0.1, shuffle=0, seed=0, ptime=0, stime=0):
    safe_mkdir(new_dir)
    st = obspy.read(old_dir)
    selected = select_traces(st, f=f, channels=channels, stations=[], 
                                  check='ps', ptime=ptime, stime=stime)
    
    stream_list = stream_to_events(selected, channels=channels, remove_duplicates=1, apply_fill=1)   
    if test:
       ...
       train_no = int((1-test)*len(stream_list))
       train_list = stream_list[:train_no]
       test_list = stream_list[train_no:] 
       stream_list = train_list
       new_dir = train_dir
       for st in test_list:
         info = get_info(st, mode='multi')
         name = make_trace_info(info)+ '.mseed'       
         st.write(os.path.join(test_dir, name), format='MSEED')
    if num: stream_list=stream_list[:num]    
    for st in stream_list:
       info = get_info(st, mode='multi')
       name = make_trace_info(info)+ '.mseed'       
...
def get_stft(data, f, norm = 1, half = 0, nperf=1, factor =1):
   """
   ...
   if half:
     factor=2
     nperf = 1
   f, t, Zxx = signal.stft(data, f, nperseg=nperf*f, noverlap=(nperf*f)/2, window=('hamming'))
...
# make title based on seismic event info
def make_trace_info(info):
   """
   This function makes a title based on seismic event info 
   ...
   if type(info['sta'])==list:
      sta=''
      for s in info['sta']:
        sta=sta+s
   else:
      sta = info['sta']
   if type(info['ch'])==list:
      chan=''
      for c in info['ch']:
        chan=chan+c
   else:
      chan = info['ch']
   label = f'{sta}_{date}T{start}to{end}_{chan}'
...
def stft_proc(stream, norm=1, f=None, filt=0, channels=[], half=0, factor=0, nperf=1, to_numpy=1, squeeze=1):
   '''
   ...
   if len(channels) > 1:
     for count, ch in enumerate(channels):
       ...
       for tr in st:
         if (ch in tr.stats.channel) and (len(stft_data) < len(channels)):
           stft_data.append(get_stft(tr.data[:-1], f, norm=norm, half=half, factor=factor, nperf=nperf))
   elif len(channels) == 1:
     ...
     for tr in st:
       stft_data.append(com.get_stft(tr.data[:-1], f, norm=norm, half=half, factor=factor, nperf=nperf))
...
def get_info(data, mode='single'):
   ...
   if hasattr(data, 'stats'):
     ...
     info['npts'] = int(info['f'][0] *(info['endt']- info['stt']))  
   return info
...
def time_format(t, symbol='\\'):
   ...
   '''
   return(f'{t.hour}{symbol}{t.minute}{symbol}{t.second}{symbol}{str(t.microsecond)[:2]}')

def date_format(t, symbol='\\'):
   '''
   ...
   '''
   return(f'{t.day}{symbol}{t.month}{symbol}{t.year}')
...
def plot_scat(val1, val2, label1, label2, valpath, offset=0.01):
     """
     ...
     ylim = get_lim(min(val2), max(val2), offset)
     #plt.plot([0, 1], [0, 1], label=f'truth', color='r', linestyle = 'dotted')     
     plt.legend()
     plt.xlabel(label1)
     plt.ylabel(label2)
     plt.ylim(ylim[0], ylim[1])
     plt.savefig(os.path.join(valpath, f'{label1}_{label2}_scatter.png'))
...
def plot_hist_wt(data, weights, label, plotdir, filename='', bins='auto', unit='', log=0, scat=0):
    """
    This function plots histogram and saves to the given directory
    ...
    if log: bins = np.logspace(np.log10(np.min(data)),np.log10(np.max(data)), bins)
    plt.figure()
    #hist = np.histogram(data, bins=np.arange(bins), weights=weights)
    #print(hist)
    #exit()
    plt.hist(data, weights=weights, label = f'{label}:{len(data)} events', histtype='step')
    if unit!= '':
      xlabel= f'{label} [{unit}]'
    else:
      xlabel= label
    ...
    plt.savefig(os.path.join(plotdir, remove_spaces(f'{filename}.png')))
    plt.close()

def plot_hist_2d(data, weights, xlabel, ylabel, plotdir, filename='', bins='auto', unit='', log=0, scat=0):
    """
    This function plots histogram and saves to the given directory
    ...
    plt.hist2d(data, weights, bins=10, label = f'{xlabel} vs. {ylabel}:{len(data)} events')
    if unit!= '':
    ...
    plt.tight_layout() 
    plt.legend()
    if filename=='': filename = xlabel_ylabel
    plt.savefig(os.path.join(plotdir, remove_spaces(f'{filename}.png')))
    plt.close()

def plot_2d(data, weights, xlabel, ylabel, plotdir, filename='', unit=''):
    """
    This function plots histogram and saves to the given directory
    ...
    plt.plot(data, weights, label = f'{xlabel} vs. {ylabel}:{len(data)} events')
    if unit!= '':
      xlabel= f'{xlabel} [{unit}]'
    else:
      xlabel= xlabel
...
def convert_to_slice_npts(slice, trig_list):
  slice_stt=slice.stats.starttime
  slice_end=slice.stats.endtime
  f = slice.stats.sampling_rate
  new_list = []
  for tm in trig_list:
    if (tm <=slice_end) and (tm >=slice_stt):
      tm_npts = (tm-slice_stt)*f
      new_list.append(tm_npts)
  return new_list  

def plot_trigger(trace, trigger, outfile='', thr_on =1, thr_off = 0):
    trig = np.zeros_like(trace.data)
    if not isinstance(trigger, list): trigger=[trigger]
    if len(trigger)==1:
      indeces = np.arange(int(trigger[0]), len(trig))
    else:
      indeces = np.arange(int(trigger[0]), int(trigger[1]))
    trig[indeces] = 1
    obspy.signal.trigger.plot_trigger(trace, trig, thr_on, thr_off)

def plot_trigger_animate(trace, trace_sum, fig, axes, count, l1, ptrigger=[], true_ptrigger=[], strigger=[], 
                         true_strigger=[], m_pred=[], t_pred=[], speaks=[]):  
    data = [trace, trace_sum] 
    #fig, axes = plt.subplots(2, 1, figsize=(8, 5))
    for ind, ax in enumerate(axes):
      ax.cla()
      pslice_npts = convert_to_slice_npts(data[ind], ptrigger)
      sslice_npts = convert_to_slice_npts(data[ind], strigger)
      true_pnpts = convert_to_slice_npts(data[ind], true_ptrigger)
      true_snpts = convert_to_slice_npts(data[ind], true_strigger)  
      
      df = data[ind].stats.sampling_rate
      npts = data[ind].stats.npts
      t = np.arange(npts, dtype=np.float32) / df
      ax.plot(t, data[ind].data, label = 'seismic data')
      i, j = ax.get_ylim()
      k, l = ax.get_xlim()
      if true_pnpts!=[]:
        
        true_pnpts=np.array(true_pnpts)/df
        
        ax.vlines(true_pnpts, i, j, color='r', lw=2,
                   label="True P onset")
        for pt in range(len(true_pnpts)):
           ax.text(1.1 * true_pnpts[pt], 0.5 * j, f'P {true_pnpts[pt]}')
      if true_snpts!=[]:
        true_snpts=np.array(true_snpts)/df
        ax.vlines(true_snpts, i, j, color='b', lw=2,
                   label="True S onset")
        for pt in range(len(true_snpts)):
           ax.text(1.1 * true_snpts[pt], 0.5 * j, f'S {true_snpts[pt]}')
      if pslice_npts!=[]:
        pslice_npts=np.array(pslice_npts)/df
        ax.vlines(pslice_npts, i, j, color='r', linestyle='--', lw=2,
                   label="P Trigger on")
        for pt in range(len(pslice_npts)):
           ax.text(1.1 * pslice_npts[pt], 0.8 * j, f'P {pslice_npts[pt]}')
        if len(m_pred)>0 :
           ax.text(0.5 * l, i  , f'Speak pred={m_pred[0]:.2E} after {t_pred[0]:.2f}s')
       
        if len(speaks['m'])>0 :
           peak, peak_t = speaks['m'][0], speaks['t'][0]
           ax.text(0.5 * l, 0.8 * i, f'actual peak={peak:.2E} after {peak_t:.2f}s')
      if sslice_npts!=[]:
        sslice_npts=np.array(sslice_npts)/df
        ax.vlines(sslice_npts, i, j, color='b', linestyle='--', lw=2,
                   label="S  Trigger on")
        for pt in range(len(sslice_npts)):
           ax.text(1.1 * sslice_npts[pt], 0.8 * j, f'S {sslice_npts[pt]}')
      ax.legend()
      ax.set_xlabel("Time after %s [s]" % trace.stats.starttime.isoformat())
      ax.set_xlim(0, np.max(t))
    plt.suptitle(trace.id)
    
def plot_trigger_ps(trace, ptrigger=[], true_ptrigger=[], strigger=[], true_strigger=[], outdir='', num=0):
    ...
    i, j = plt.ylim()
    if true_ptrigger!=[]:
      true_ptrigger=np.array(true_ptrigger)/df
      plt.vlines(true_ptrigger, i, j, color='r', lw=2,
                   label="True P onset")
    if true_strigger!=[]:
      true_strigger=np.array(true_strigger)/df
      plt.vlines(true_strigger, i, j, color='b', lw=2,
                   label="True S onset")
    if ptrigger!=[]:
      ptrigger=np.array(ptrigger)/df
      plt.vlines(ptrigger, i, j, color='r', linestyle='--', lw=2,
                   label="P Trigger on")
    if strigger!=[]:
      strigger=np.array(strigger)/df
      plt.vlines(strigger, i, j, color='b', linestyle='--', lw=2,
                   label="S  Trigger on")
    
    plt.legend()
    plt.xlabel("Time after %s [s]" % trace.stats.starttime.isoformat())
    plt.suptitle(trace.id)
    #plt.show()
    if num==0: name = 'event'
    fig.savefig(os.path.join(outdir, f'event{str(num)}.png'))
    plt.close(fig)
################################### main ##########################################################
