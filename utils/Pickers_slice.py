"""
This class implements pickers defined by obspy applied to data slices
"""
import numpy as np
...
class Pickers():
   ...
   def true_picks(self, stream, ptime, stime):
     """
     This function prints P and S arrivals and returns the values for the whole stream
   ...
   def post_proc_truth(self, batch_data, prediction, count, tm, stt, f_act, picks_dict, triggers): 
     ...
     picks_dict[self.mode]['s']['utc'].append(stime)
     picks_dict[self.mode]['s']['value'].append(1.0)
     picks_dict[self.mode]['s']['pick'].append(com.get_loc(stime, stt, f_act)) 
   ...
   def sta_lta(self, stream, ptime=0, stime=0, sta_time=5, lta_time=0):
     ...
     returns utc datetime for p and s arrivals
     """
     if lta_time==0: lta_time=self.win # use the window (20 sec for LTA)
   ...
   def post_proc_sta_lta(self, batch_data, prediction, count, tm, stt, f_act, picks_dict, triggers): 
     ...
     prediction: picker output sta\lta
     ...
     picks_dict[self.mode]['p']['out'].append(1.0)  
     picks_dict[self.mode]['p']['out_t'].append(tm) 
     picks_dict[self.mode]['p']['out_loc'].append(com.get_loc(tm, stt, f_act)) 
   ...
   def post_proc_pkbaer(self, batch_data, prediction, count, tm, stt, f_act, picks_dict, triggers):  
     ...
     if ppick > 0:
       picks_dict[self.mode]['p']['utc'].append(ptime)
       picks_dict[self.mode]['p']['value'].append(1.0)
       picks_dict[self.mode]['p']['pick'].append(ppick) 
     ...
     picks_dict[self.mode]['s']['value']= [0.0]
     picks_dict[self.mode]['s']['pick']= [0.0] 
     picks_dict[self.mode]['s']['out'].append(1.0)  
     picks_dict[self.mode]['s']['out_t'].append(tm) 
     picks_dict[self.mode]['s']['out_loc'].append(com.get_loc(tm, stt, f_act)) 
     return picks_dict, triggers

   # apply ar pick from obspy
   def apply_arpick(self, stream):
     """
   ...
   def post_proc_arpick(self, batch_data, prediction, count, tm, stt, f_act, picks_dict, triggers): 
     ...
     if spick > 0:
       picks_dict[self.mode]['s']['utc'].append(stime)
       picks_dict[self.mode]['s']['value'].append(1.0)
       picks_dict[self.mode]['s']['pick'].append(spick) 

     pval = 1 if ppick > 0 else 0
     sval = 1 if spick > 0 else 0

     picks_dict[self.mode]['p']['out'].append(pval)  
     picks_dict[self.mode]['p']['out_t'].append(tm) 
     picks_dict[self.mode]['p']['out_loc'].append(com.get_loc(tm, stt, f_act))
     picks_dict[self.mode]['s']['out'].append(sval)  
     picks_dict[self.mode]['s']['out_t'].append(tm) 
     picks_dict[self.mode]['s']['out_loc'].append(com.get_loc(tm, stt, f_act))  
