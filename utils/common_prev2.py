...
import numpy as np
import obspy
import obspy.signal.trigger as trig
...
def get_stft(data, f, norm = 1, nperf=1, half=1, factor =0):
   """
   ...
   data_len = len(data)
   nperseg=int(nperf * f)
   print(nperseg, nperseg//4)
   
   f, t, Zxx = signal.stft(data, f, nperseg=nperseg, window=('hamming'))
...
def format(data_path, new_path):
  st = obspy.read(datapath)
  com.safe_mkdir(new_path)
  for tr in st[::]:
    info = get_info(tr)
    name = make_trace_info(info)+ '.mseed'
    tr.write(os.path.join(new_path, name), format='MSEED')
...
def stft_proc(stream, norm=1, f=None, filt=0, channels=[], factor=2, nperf=0, half=1, to_numpy=1, squeeze=1):
   '''
   ...
   if len(channels) > 1:
     for count, ch in enumerate(channels):
       ...
       for tr in st:
         if (ch in tr.stats.channel) and (len(stft_data) < len(channels)):
           stft_data.append(get_stft(tr.data[:-1], f, norm=norm, nperf=nperf, half=half, factor=factor))
   elif len(channels) == 1:
     ...
     for tr in st:
       stft_data.append(com.get_stft(tr.data[:-1], f, norm=norm, nperf=nperf, half=half, factor=factor))
...
def plot_trigger(tr, ptime, stime, file=None):
   ...
   print(ptime, stime)
   print(int(ptime *f), int(tr_len - (stime*f)), tr_len)
   #exit()
   ...
   if file:
     trig.plot_trigger(tr, ph_trigger,0.5, 0.5, outfile=file)
   else:
     trig.plot_trigger(tr, ph_trigger,0.5, 0.5)
...
def plot_trigger2(trace, trigger, outfile='', thr_on =1, thr_off = 0):
...
def plot_trigger_ps(trace, ptrigger=[], true_ptrigger=[], strigger=[], true_strigger=[], outdir='', name=''):
