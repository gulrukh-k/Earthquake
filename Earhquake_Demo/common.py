...
def plot_trigger_animate(trace, trace_sum, fig, axes, count, l1, ptrigger=[], true_ptrigger=[], strigger=[], 
                         true_strigger=[], m_pred=[], t_pred=[], speaks=[], colors='dark'):  
    data = [trace, trace_sum]
    textcolor= 'white' if colors=='dark' else 'white'
    for ind, ax in enumerate(axes):
      ax.cla()
      if colors=='dark': ax.set_facecolor("black")
      ...
      ax.plot(t, data[ind].data, label = 'seismic data', color='#00FF00')
      i, j = ax.get_ylim()
      k, l = ax.get_xlim()
      if true_pnpts!=[]:
        ...
        for pt in range(len(true_pnpts)):
           ax.text(1.1 * true_pnpts[pt], 0.5 * j, f'P {true_pnpts[pt]}', color='r')
      if true_snpts!=[]:
        true_snpts=np.array(true_snpts)/df
        ax.vlines(true_snpts, i, j, color='b', lw=2,
                   label="True S onset")
        for pt in range(len(true_snpts)):
           ax.text(1.1 * true_snpts[pt], 0.5 * j, f'S {true_snpts[pt]}', color='b')
      if pslice_npts!=[]:
        ...
        if len(m_pred)>0 :
           ax.text(0.5 * l, i  , f'Speak pred={m_pred[0]:.2E} after {t_pred[0]:.2f}s', color=textcolor)
       
        if len(speaks['m'])>0 :
           peak, peak_t = speaks['m'][0], speaks['t'][0]
           ax.text(0.5 * l, 0.8 * i, f'actual peak={peak:.2E} after {peak_t:.2f}s', color=textcolor)
      if sslice_npts!=[]:
        sslice_npts=np.array(sslice_npts)/df
        ax.vlines(sslice_npts, i, j, color='b', linestyle='--', lw=2,
                   label="S  Trigger on")
        for pt in range(len(sslice_npts)):
           ax.text(1.1 * sslice_npts[pt], 0.8 * j, f'S {sslice_npts[pt]}', color='b')
      ax.legend()
